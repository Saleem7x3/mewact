...
To verify the installation and see the Agent in action without Claude:

1. **Simple Integration Test**:
   ```bash
   python mcp_demo.py
   ```
   *Opens Notepad, types text, and captures screen.*

2. **Complex Multi-App Workflow**:
   ```bash
   python mcp_complex_demo.py
   ```
   *Retreives System Info, uses Calculator, and transfers data via clipboard.*
