...
# Format: &&$47 <ID> <command> [| <var>] $&47
# OR with var ref: &&$47 <ID> <command> $V<num> $&47
TRIGGER_PATTERN = r"[&]{2}[\$S]47\s*(\d+)\.?\s*(.*?)\s*[\$S]{2}[&]47"
    ...
    def plan(self, goal: str, ui_data: List[Dict]) -> Tuple[str, bool]:
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                last_error = e
                if attempt < max_retries:
                    print(f"{Fore.YELLOW}[!] LLM call failed: {type(e).__name__}, retrying... ({attempt + 1}/{max_retries})")
...
# --- 5. SENTINEL ---
# Regex for detecting partial triggers (start without end)
PARTIAL_TRIGGER_START = r"[&]{2}[\$S]47\s*(\d+)\.?\s*"
...
class PassiveSentinel:
    ...
    def _check_pending_triggers(self, current_txt: str, buffered_text: str):
        ...
        start_pattern = r"[&]{2}[\$S]47\s*(\d+)\.?\s*([^\$]*?)$"
        end_pattern = r"^(.*?)[\$S]{2}[&]47"
        ...
        if start_match:
            ...
            if cid not in self.pending_triggers and cid not in self.executed_ids:
                ...
                full_check = re.search(TRIGGER_PATTERN + r".*?" + re.escape(partial_cmd), buffered_text)
                if not full_check:
        ...
        for cid in list(self.pending_triggers.keys()):
            # Look for end delimiter in current text
            end_match = re.search(r"(.*?)[\$S]{2}[&]47", current_txt)
            if end_match:
